...
export default function ExamPage() {
  ...
  useEffect(() => {
    // Lấy dữ liệu đề thi từ file exam_db.json
    fetch(`/exam_db.json?v=${new Date().getTime()}`)
      ...
      .then((data) => {
        ...
        if (foundExam) {
          ...
          router.push("/"); // Nếu không thấy đề thi, quay về trang chủ
        }
      });
  }, [id, router]);

  if (!exam) return <div className="flex h-screen items-center justify-center text-gray-500 font-medium">Đang tải dữ liệu đề thi...</div>;

  const currentQuestion = exam.questions[currentQIndex];

  // Hàm chọn đáp án
  const handleSelectOption = (optionIndex: number) => {
    if (isSubmitted) return; // Nếu đã nộp bài thì không cho sửa
    setSelectedAnswers({
      ...selectedAnswers,
  ...
  const handleSubmit = () => {
    if (window.confirm("Bạn có chắc chắn muốn nộp bài không?")) {
      ...
      setIsSubmitted(true);
      setCurrentQIndex(0); // Quay về câu 1 để xem lại từ đầu
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f8f9fa] text-gray-800 font-sans">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* CỘT TRÁI: DANH SÁCH CÂU HỎI (QUICK NAV) */}
          ...
          <div className="grid grid-cols-4 gap-2">
            {exam.questions.map((_: any, idx: number) => {
              ...
              if (!isSubmitted) {
                ...
                if (idx === currentQIndex) btnClass += "ring-2 ring-offset-2 ring-indigo-500 "; // Highlight câu đang xem
        ...
        {/* CỘT PHẢI: KHUNG LÀM BÀI / XEM GIẢI THÍCH */}
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mb-8">
              ...
              <div className="space-y-3">
                {currentQuestion.options.map((opt: string, optIdx: number) => {
                  ...
                  let icon = <div className="w-5 h-5 rounded-full border-2 border-gray-300 shrink-0 mt-0.5" />; // Vòng tròn rỗng mặc định
                  ...
                  if (!isSubmitted) {
                    ...
                    // Trạng thái đã nộp bài (XEM LẠI)
                    const isCorrectOption = optIdx === currentQuestion.correctIndex;
                    
                    if (isCorrectOption) {
                      // Bôi xanh đáp án đúng
                      optionClass += "border-green-500 bg-green-50 text-green-900 font-bold z-10 relative";
              ...
              {/* HỘP HIỂN THỊ GIẢI THÍCH (Chỉ hiện khi đã nộp bài) */}
              {isSubmitted && currentQuestion.explain && (
                <div className="mt-8 animate-fade-in">
                  <div className="bg-[#eff6ff] border-l-4 border-[#3b82f6] p-5 rounded-r-xl shadow-sm">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h4 className="font-bold text-[#1e40af] text-sm uppercase tracking-wider">Giải thích chi tiết</h4>
                    </div>
                    <p className="text-sm text-[#1e3a8a] leading-relaxed ml-7">
